...
_CAMERA_K = np.array([
    [608.3013305664062, 0.0, 324.0627746582031],
    [0.0, 608.1605224609375, 248.2197723388672],
    [0.0, 0.0, 1.0]
])
...
def transform_camera_pose_to_base(x, y, z, theta,
                                  camera_frame='camera_link',
                                  base_frame='BASE_S',
                                  timeout=1.0,
                                  theta_in_degrees=False):
    """
    输入相机坐标系下的位置(x,y,z)，将其转换到base坐标系；
    同时在base坐标系下绕X轴旋转theta，返回末端执行器的四元数。

    返回字典: {'x','y','z','ox','oy','oz','ow'}
    其中(x,y,z)为base坐标系下位置，(ox,oy,oz,ow)为在base坐标系下绕X轴转theta的四元数。
    """
    ...
    try:
        ...
        rospy.logerr(f"TF lookup failed ({camera_frame} -> {base_frame}): {str(e)}")
        return None
    ...
    # 在base坐标系下：先绕Z轴旋转90度，再绕X轴旋转theta
    if theta_in_degrees:
        theta = np.deg2rad(float(theta))
    else:
        theta = float(theta)
    q_z = tf_trans.quaternion_about_axis(np.deg2rad(90.0), (0.0, 0.0, 1.0))
    q_x = tf_trans.quaternion_about_axis(theta, (1.0, 0.0, 0.0))
    q_total = tf_trans.quaternion_multiply(q_x, q_z)
    ox, oy, oz, ow = q_total
...
def move_along_orientation_from_pose(position, orientation, distance=0.1):
    ...
    print(f"Successfully calculated position")
    return new_position
...
def get_move_group(group_name):
    ...
    if group_name not in _move_groups:
        try:
            ...
            rospy.logerr(f"Failed to create MoveGroupCommander for group '{group_name}': {str(e)}")
            return None
...
def get_current_pose_http():
    try:
        ...
        rospy.logerr(f"Error in get_current_pose_http: {str(e)}")
        return jsonify({
            "success": False,
            "error": f"Internal server error: {str(e)}"
        }), 500

@app.route('/calculate_out_position', methods=['POST'])
def calculate_out_position():
    try:
        data = request.get_json()
        if not data:
            return jsonify({
        ...
        # 调用计算函数
        new_position = move_along_orientation_from_pose(position, orientation, distance)
        if new_position is None:
            return jsonify({
                "success": False,
                "error": f"Failed to calculate position: {error}"
            }), 500
        ...
        response_data = {
            ...
            "pose": {
                "position": {
                    "x": round(new_position[0], 6),
                    "y": round(new_position[1], 6),
                    "z": round(new_position[2], 6)
                },
                "orientation": {
...
def plan_to_position():
    try:
        ...
        if 'group_name' not in data:
            return jsonify({"success": False, "message": "Missing group_name"}), 400
        if 'position' not in data:
            return jsonify({"success": False, "message": "Missing position"}), 400
        if 'orientation' not in data:
            return jsonify({"success": False, "message": "Missing orientation"}), 400
        ...
        move_group = get_move_group(group_name)
        if move_group is None:
            return jsonify({"success": False, "message": f"Invalid group_name: {group_name}"}), 400
        ...
        # 如果启用直线约束，创建路径约束以保持当前姿态
        if is_straight_constraint:
            # 获取当前末端执行器姿态
            current_pose = move_group.get_current_pose().pose
            # 创建四元数约束，保持当前旋转姿态不变
            quaternion_stamped = geometry_msgs.msg.QuaternionStamped()
            quaternion_stamped.header.frame_id = "BASE_S"  # 基坐标系
            quaternion_stamped.quaternion = current_pose.orientation
            # 获取末端执行器链接名称
            ...
            print("Applied straight line constraint to maintain current orientation")
        ...
        if joint_values is None:
            ...
            return jsonify({"success": False, "message": "IK solver failed - no solution found"}), 500

        # 使用关节空间规划到达目标
        print("Starting joint space motion planning and execution")
        success = plan_to_joint_target(move_group, joint_values, joint_names)
        if success:
            print("Motion planning and execution successful!")
            return jsonify({"success": True, "message": "Plan completed successfully"}), 200
        else:
            print("Motion planning or execution failed!")
            return jsonify({"success": False, "message": "Motion planning or execution failed"}), 500        

    except Exception as e:
        print(f"Error in data_trans: {str(e)}")
