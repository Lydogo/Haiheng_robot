...
//control_msgs::FollowJointTrajectoryGoalConstPtr is 
void execute_callback(const control_msgs::FollowJointTrajectoryGoalConstPtr& goalPtr, Server* moveit_server)
{   
    //
    ROS_INFO("Received a new goal, starting to process trajectory.");
    ...
    // cout << moveit_tra;
    ROS_INFO("Printing trajectory data:");
    for (size_t i = 0; i < moveit_tra.joint_trajectory.points.size(); ++i)
    {
        std::ostringstream ss;
        ss << "Point " << i << ": ";
        for (size_t j = 0; j < moveit_tra.joint_trajectory.joint_names.size(); ++j)
        {
            ss << moveit_tra.joint_trajectory.joint_names[j] << "="
            << moveit_tra.joint_trajectory.points[i].positions[j] << " ";
        }
    ...
    moveit_server->setSucceeded(result);
}
...
int main(int argc, char *argv[])
{
    ...
    Server right_arm_server(nh,"right_arm_controller/follow_joint_trajectory", boost::bind(&execute_callback, _1, &right_arm_server), false);
    Server left_arm_server(nh,"left_arm_controller/follow_joint_trajectory", boost::bind(&execute_callback, _1, &left_arm_server), false);
    // 手动启动服务器
    right_arm_server.start();
    left_arm_server.start();
